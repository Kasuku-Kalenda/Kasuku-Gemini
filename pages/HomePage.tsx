...
export const HomePage: React.FC<HomePageProps> = ({ onViewEvent, navigateToModule, scrollToId, onScrolled }) => {
  ...
  const handleQueryChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newQuery.length > 1) {
      const filtered = allEvents.filter(ev => ev.title.toLowerCase().includes(newQuery.toLowerCase())).slice(0, 5);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center p-8 bg-card rounded-2xl shadow-soft">
        <h1 className="text-4xl md:text-5xl font-bold text-secondary">Découvrez l'Histoire, un jour à la fois</h1>
      ...
      {isLoading ? (
          ...
          <div className="space-y-2">
            {sortedEvents.map(event => (
              ...
              >
                ...
                {/* Save */}
                <button
                  className="no-min-h shrink-0 p-2.5 rounded-full transition-all active:scale-90 text-muted-foreground hover:text-primary hover:bg-primary/10"
                  onClick={e => {
                    e.stopPropagation();
        ...
        <div className="col-span-full text-center py-16 bg-card rounded-2xl shadow-soft">
